...
#Theoriekurve
Frequenz_neu = np.genfromtxt('WerteTheo1.txt', unpack=True)
U_diff_theo =[ 1 / ((1 - L * C * x**2)**2 + x**2 * R2**2 * C**2)**(0.5) for x  in Frequenz_neu]
print(U_diff_theo)
U_diff_theo_nutz = np.genfromtxt('Udiffneu.txt', unpack=True)
...
plt.plot(Frequenz_cd, U_diff_theo_nutz, 'r-', label = 'Theorie')
plt.xlabel('f in $kHz$')
plt.ylabel('$U$ / $U_0$')
plt.legend()
...
f_res_gemessen = 33000 #in kHz, Frequenz zum Maximum aus Plot2
...
q_theo_neu = (w_0 * L)/(R2 + R_N)
q_exp_neu = (f_res_gemessen ) / 11140
print(q_theo_neu, q_exp_neu)
plt.plot(Frequenz_cd, U_diff, 'k.', label = 'Messdaten')
plt.xlabel('f in $kHz$')
plt.ylabel('$U$ / $U_0$')
...
z=(np.linspace(14000,34000,20)*2*math.pi*R2.nominal_value*C.nominal_value)/(1-L.nominal_value*C.nominal_value*np.linspace(14000,34000,20)**2*4*math.pi**2)
print(z)
y=(np.linspace(35000,50000,20)*2*math.pi*R2.nominal_value*C.nominal_value)/(1-L.nominal_value*C.nominal_value*np.linspace(35000,50000,20)**2*4*math.pi**2)
print(y)
plt.plot(np.linspace(14000,34000,20),np.arctan(z), 'b--', label = 'Theorie')
plt.plot(np.linspace(35000,50000,20),np.arctan(y)+3.3, 'b--')
